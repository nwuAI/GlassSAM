...
def get_boxes_from_mask(mask, box_num=1, std = 0.1, max_pixel = 5):
    ...
    # duplicate the existing boxes
        ...
        if len(boxes) == 0:
            default_box = (0, 0, 1, 1)  # 假设这是一个默认的 1x1 大小的框
            boxes = [default_box] * num_duplicates  # 创建 num_duplicates 个默认框

        # 如果 boxes 不为空，复制现有的框来填充 num_duplicates
        elif len(boxes) > 0:
            boxes += [boxes[i % len(boxes)] for i in range(num_duplicates)]
...
def select_points_triple(pr, gt, point_num=9, ratio=(0.33, 0.33, 0.34), replace_if_needed=True):
    """
    pr, gt: torch.Tensor, shape [B,H,W] or [H,W]
            Note: here they are binarized cross-modal features (not prediction/GT).
    point_num: total number of points per image
    ratio: (fg_ratio, bg_ratio, un_ratio) sampling proportion
    replace_if_needed: if a class has too few pixels, allow sampling with replacement
    Returns:
        batch_points: np.array [B, point_num, 2] in (x, y) format for SAM -> (col, row)
        batch_labels: np.array [B, point_num] labels in {0,1,-1}
    """
    ...
    # number per class
    fg_n = int(round(point_num * ratio[0]))
    bg_n = int(round(point_num * ratio[1]))
    un_n = point_num - fg_n - bg_n  # ensure sum equals point_num

    batch_points, batch_labels = [], []

    def _sample_from_indices(indices, k):
        """indices: array [N,2] with (row, col). Return sampled [k,2]."""
        if indices.shape[0] == 0:
            return None
        rep = replace_if_needed or (indices.shape[0] < k)
        choice = np.random.choice(indices.shape[0], k, replace=rep)
    ...
    for j in range(B):
        ...
        # ---- define three regions (row, col)
        fg_idx = np.argwhere((one_pr == 1) & (one_gt == 1))  # foreground agreement
        bg_idx = np.argwhere((one_pr == 0) & (one_gt == 0))  # background agreement
        un_idx = np.argwhere(one_pr != one_gt)               # disagreement (uncertain)

        sel_fg = _sample_from_indices(fg_idx, fg_n)
        sel_bg = _sample_from_indices(bg_idx, bg_n)
        sel_un = _sample_from_indices(un_idx, un_n)

        # ---- fallback if any class empty: sample from the union
        all_idx = np.argwhere(np.ones((H, W), dtype=bool))
        def _fallback_fill(sel, k):
            if sel is None:
                # sample anywhere if the region is empty
                return _sample_from_indices(all_idx, k)
            return sel

        sel_fg = _fallback_fill(sel_fg, fg_n)
        sel_bg = _fallback_fill(sel_bg, bg_n)
        sel_un = _fallback_fill(sel_un, un_n)

        selected = np.concatenate([sel_fg, sel_bg, sel_un], axis=0)  # [point_num,2]
        # optional shuffle so order doesn't leak class
        np.random.shuffle(selected)

        points, labels = [], []
        for r, c in selected:  # r=row, c=col
            # convert to SAM coords: (x, y) = (col, row)
            points.append((int(c), int(r)))

            # label assignment consistent with your design
            if one_pr[r, c] == 0 and one_gt[r, c] == 0:
                labels.append(0)     # background
            elif one_pr[r, c] == 1 and one_gt[r, c] == 1:
    ...
    return np.array(batch_points, dtype=np.int32), np.array(batch_labels, dtype=np.int32)
...
def select_random_points(pr, gt, point_num = 9):
    ...
    # pred, gt = pr.data.cpu().numpy(), gt.data.unsqueeze(1).cpu().numpy()
    ...
    batch_points = []
    batch_labels = []
    # print("pred, gt, error_shape", pred.shape, gt.shape, error.shape)
...
def init_point_sampling_glass(mask, get_point=1):
    ...
    if get_point == 1:
        ...
        # num_fg = min(get_point // 2, fg_size)
        # num_bg = min(get_point - num_fg, bg_size)
        num_fg = get_point // 2
...
def test_transforms_glass_union(img_size, ori_h, ori_w):
    H, W = img_size
    tfms = []
    if ori_h < H and ori_w < W:
        tfms.append(A.PadIfNeeded(min_height=H, min_width=W,
                                 border_mode=cv2.BORDER_CONSTANT, value=0, mask_value=0))
    else:
        tfms.append(A.Resize(H, W, interpolation=cv2.INTER_LINEAR))
    tfms.append(ToTensorV2(p=1.0))
    return A.Compose(tfms, p=1.0, additional_targets={'aux': 'image', 'mask': 'mask'})
...
def build_train_transforms_union(img_size):
    H, W = img_size
...
def train_transforms_aug(img_size, ori_h, ori_w):
    ...
    return A.Compose(transforms, p=1.0, additional_targets={'depth': 'image', 'mask': 'mask', "aolp":"image"})
...
def generate_point(masks, labels, low_res_masks, batched_input, point_num):
    ...
    # masks, labels, low_res_masks = masks.squeeze(), labels.squeeze(), low_res_masks.squeeze()
    # print("masks, labels, low_res_masks", masks.shape, labels.shape, low_res_masks.shape)
