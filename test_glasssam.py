...
from DataLoader_new_cat import TestingDataset_Glass_union
from metrics import SegMetrics
import time
from tqdm import tqdm
import numpy as np
from torch.nn import functional as F
from utils import init_point_sampling_mirror, get_boxes_from_mask_mirror, select_random_points, select_points_triple
import logging
import datetime
import cv2
import random
...
def parse_args():
    ...
    parser.add_argument("--work_dir", type=str, default="/home/xxxxx/", help="work dir")
    parser.add_argument("--run_name", type=str, default="predict_bestpth", help="run model name")
    parser.add_argument("--batch_size", type=int, default=1, help="batch size")
    parser.add_argument("--image_size", type=int, default=(416, 416), help="image_size")
    parser.add_argument('--device', type=str, default='cuda:2')
    parser.add_argument("--data_path", type=str, default="/home/xxxxx/dataset/RGBP-Glass", help="train data path")
    parser.add_argument("--metrics", nargs='+', default=['iou', 'dice', 'mae', 'ber'], help="metrics")
    parser.add_argument("--model_type", type=str, default="vit_b", help="sam model_type")
    parser.add_argument("--sam_checkpoint", type=str, default="/home/xxxxx/sam_lastiou.pth", help="sam checkpoint")
...
def prompt_and_decoder(args, batched_input, depth_decoder, ddp_model, image_embeddings):
...
def main(args):
    ...
    test_dataset = TestingDataset_Glass_union(data_path=args.data_path, image_size=args.image_size, mode='test', requires_name=True, point_num=args.point_num, return_ori_mask=True, prompt_path=args.prompt_path)
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        with torch.no_grad():
            ...
            depth_control, a_x, r_edge, r_loacal = model.xmda(depth_control_rgb, depth_control_dep)
            ...
            points, point_labels = select_points_triple(depth_sup_rgb_binary.squeeze(), depth_sup_dep_binary.squeeze(), point_num=args.point_num)
        ...
        if args.boxes_prompt:
            ...
            masks, low_res_masks, iou_predictions = prompt_and_decoder(args, batched_input, depth_sup, model, image_embeddings)
            points_show = None

        else:
            save_path = os.path.join(f"{args.work_dir}", args.run_name, f"iter{args.iter_point if args.iter_point > 1 else args.point_num}_predict")
    ...
    if args.prompt_path is None:
        with open(os.path.join(args.work_dir,f'{args.image_size}_prompt_dolp_triple.json'), 'w') as f:
